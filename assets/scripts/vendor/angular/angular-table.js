      ...
      return {
        ...
        link: function($scope, $element, $attributes) {
          ...
          $scope.goToPage = function(page) {
            ...
            var data = $scope.pageSequence.data,
                maxs = w.getMaxPages()/2;
            if (page === data[0])
                $scope.pageSequence.relocate(-maxs);
            if (page === $scope.pageSequence.data[data.length-1])
                $scope.pageSequence.relocate(maxs);
            if (page < $scope.pageSequence.data[maxs]) 
                $scope.pageSequence.relocate(-1);
            if (page > $scope.pageSequence.data[maxs])
                $scope.pageSequence.relocate(1);
          };
